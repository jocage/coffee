...
const methods: Array<BrewMethod | "all"> = ["all", "V60", "Origami", "Kalita", "AeroPress", "Espresso", "French Press", "Switch"];
const visibilities: Array<Visibility | "all"> = ["all", "public", "followers", "unlisted", "private"];

export default async function ExplorePage({ searchParams }: { searchParams: Promise<{ q?: string; method?: string; visibility?: string; tab?: string }> }) {
  const params = await searchParams;
  const method = methods.includes(params.method as BrewMethod) && params.method !== "all" ? (params.method as BrewMethod) : undefined;
  const visibility = visibilities.includes(params.visibility as Visibility) ? (params.visibility as Visibility | "all") : "all";
  const tab = params.tab ?? "recipes";
  const [recipes, profiles, coffees, gear, clubs] = await Promise.all([
    searchRecipes({ query: params.q, method, visibility }),
    getProfiles(),
    getCoffees(),
    getGear(),
    getClubs()
  ...
  const filteredProfiles = query
    ? profiles.filter((profile) => [profile.displayName, profile.handle, profile.bio, profile.location].some((value) => value.toLowerCase().includes(query)))
    : profiles;
  const filteredCoffees = query
    ? coffees.filter((coffee) =>
        [coffee.name, coffee.roaster, coffee.origin, coffee.process, coffee.flavorNotes.join(" ")].some((value) => value.toLowerCase().includes(query))
      )
    : coffees;
  const filteredGear = query
    ? gear.filter((item) => [item.name, item.brand, item.model, item.notes, item.type].some((value) => value.toLowerCase().includes(query)))
    : gear;
  const filteredClubs = query
    ? clubs.filter((club) => [club.name, club.description, club.slug].some((value) => value.toLowerCase().includes(query)))
    : clubs;
    ...
    <div className="mx-auto grid max-w-7xl gap-5 px-4 py-5 md:px-6 lg:grid-cols-[260px_1fr]">
      <aside className="hidden lg:block">
        <Card className="sticky top-24">
          ...
          <FilterForm q={params.q} method={method ?? "all"} visibility={visibility} />
        </Card>
      </aside>
      <section>
        <div className="mb-5 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-[var(--text-muted)]">Find recipes, people, beans, gear and clubs.</p>
          </div>
        ...
        <form className="mb-4 grid gap-3 md:grid-cols-[1fr_auto_auto]" action="/explore">
          ...
          <Input name="q" defaultValue={params.q} placeholder="Search recipes, people, flavor notes..." aria-label="Search explore" />
          <Select name="method" defaultValue={method ?? "all"} aria-label="Filter by brew method" className="md:w-44">
            {methods.map((item) => (
        ...
        {tab === "recipes" ? (
          ...
          filteredProfiles.length > 0 ? <PeopleGrid profiles={filteredProfiles} /> : <EmptySearchState />
        ) : tab === "beans" ? (
          filteredCoffees.length > 0 ? <CoffeeGrid coffees={filteredCoffees} /> : <EmptySearchState />
        ) : tab === "gear" ? (
          filteredGear.length > 0 ? <GearGrid gear={filteredGear} /> : <EmptySearchState />
        ) : tab === "clubs" ? (
          filteredClubs.length > 0 ? <ClubGrid clubs={filteredClubs} /> : <EmptySearchState />
        ) : (
          <EmptySearchState />
        )}
      </section>
...
function FilterForm({ q, method, visibility }: { q?: string; method: BrewMethod | "all"; visibility: Visibility | "all" }) {
  return (
    <form action="/explore" className="mt-5 grid gap-4 text-sm">
      ...
      <Button type="submit" variant="secondary" icon={<SlidersHorizontal className="h-4 w-4" aria-hidden />}>
        Apply filters
      </Button>
    </form>
  );
  ...
  return (
    <Card className="grid min-h-56 place-items-center text-center">
      <div>
        ...
        <p className="mt-2 text-sm text-[var(--text-muted)]">Try a broader search, a different method, or reset visibility.</p>
      </div>
    </Card>
  );
}
  ...
  return (
    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
      {profiles.map((profile) => (
        <Link key={profile.id} href={`/u/${profile.handle}`} className="focus-ring rounded-[var(--radius-md)]">
      ...
      {coffees.map((coffee) => (
        <Card key={coffee.id} className="overflow-hidden p-0">
          ...
          <div className="p-4">
            ...
            <p className="mt-3 line-clamp-2 text-sm text-[var(--text-dim)]">{coffee.flavorNotes.join(", ")}</p>
          </div>
        </Card>
      ))}
    </div>
          ...
          <Card className="h-full overflow-hidden p-0">
            ...
            <div className="p-4">
              <div className="flex items-center justify-between gap-3">
                ...
                <span className="text-xs text-[var(--text-dim)]">{club.memberCount.toLocaleString()} members</span>
              </div>
              <h2 className="serif mt-3 text-2xl">{club.name}</h2>
              <p className="mt-2 line-clamp-3 text-sm text-[var(--text-muted)]">{club.description}</p>
            </div>
          </Card>
        </Link>
      ))}
