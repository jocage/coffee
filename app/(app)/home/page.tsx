import { BookOpen, Coffee, Plus, Settings } from "lucide-react";
    ...
    <div className="mx-auto grid w-full max-w-7xl gap-5 overflow-x-hidden px-4 py-5 md:px-6 lg:grid-cols-[minmax(0,1fr)_360px]">
      <section className="grid min-w-0 gap-5">
        <Card className="relative min-w-0 overflow-hidden p-0">
          <Image src={heroImage} alt="" fill priority sizes="900px" className="object-cover opacity-60" />
          ...
          <div className="relative z-10 w-full max-w-xl p-6 md:p-10">
            ...
            <h1 className="serif mt-4 text-4xl leading-none sm:text-5xl md:text-6xl">Every drop tells your story.</h1>
            <p className="mt-4 text-[var(--text-muted)]">Track brews. Refine recipes. Share your craft.</p>
          ...
          <Card className="min-w-0">
            ...
            {dashboard.focusRecipe ? (
              <>
                ...
                <p className="mt-2 text-sm text-[var(--text-muted)]">{dashboard.focusRecipe.subtitle}</p>
                <dl className="mt-6 grid gap-3 text-sm">
                  ...
                  <div className="flex justify-between">
                    ...
                    <dd>{dashboard.focusRecipe.temperatureCelsius}C / {dashboard.focusRecipe.waterGrams}g</dd>
                  </div>
                </dl>
          ...
          <div className="grid min-w-0 gap-5">
            <Card className="min-w-0">
              ...
              <div className="mt-4 grid grid-cols-2 gap-3 md:grid-cols-4">
                {[
                  ["/brews/new", Plus, "New Brew"],
                  ["/recipes/new", BookOpen, "New Recipe"],
                  ["/gear/grinders/new", Settings, "Add Gear"],
                  ["/coffees/new", Coffee, "Add Coffee"]
                ].map(([href, Icon, label]) => (
                  <Link key={String(label)} href={String(href)}>
                    <Button variant="secondary" className="h-20 w-full flex-col" icon={<Icon className="h-5 w-5" aria-hidden />}>
                      {String(label)}
                    </Button>
                  </Link>
                ))}
            ...
            <Card className="min-w-0">
              <CardHeader>
                ...
                <Link href="/brews" className="text-sm text-[var(--accent)]">View all</Link>
              </CardHeader>
              {dashboard.recentBrews.length > 0 ? (
                ...
                <EmptyPanel title="No brews logged" body="Log a brew to track ratings, grind changes and tasting notes." actionHref="/brews/new" actionLabel="Log brew" compact />
              )}
            </Card>
          </div>
        </div>
        ...
        <Card className="min-w-0">
          ...
          <div className="grid gap-4">
            {feed.length > 0 ? (
              ...
              <EmptyPanel title="No public activity yet" body="Follow brewers or publish a recipe to build your feed." actionHref="/explore" actionLabel="Explore" compact />
            )}
          </div>
        </Card>
      </section>
      ...
      <aside className="grid min-w-0 content-start gap-5">
        <Card>
          ...
          {dashboard.favoriteRecipes.length > 0 ? (
            ...
            <EmptyPanel title="Nothing saved" body="Save recipes from Explore to collect references for later brews." actionHref="/explore" actionLabel="Find recipes" compact />
          )}
        </Card>
        {dashboard.focusRecipe ? (
          <Card>
        ...
        <Card>
          ...
          <ol className="mt-4 space-y-3 text-sm text-[var(--text-muted)]">
            {["Ethiopia natural clarity", "Blooming tips", "Low agitation pours", "High extraction Kalita"].map((item, index) => (
              <li key={item} className="flex items-center gap-3">
                <span className="grid h-6 w-6 place-items-center rounded-full bg-white/8 text-xs text-[var(--accent)]">{index + 1}</span>
                {item}
              </li>
            ))}
          </ol>
...
function EmptyPanel({
  ...
  return (
    <div className={compact ? "rounded-[var(--radius-sm)] border border-dashed border-[var(--border)] p-4" : "mt-6 rounded-[var(--radius-sm)] border border-dashed border-[var(--border)] p-5"}>
