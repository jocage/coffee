...
function Bubble({ body, align }: { body: string; align: "left" | "right" }) {
  return (
    <p className={align === "right" ? "ml-auto max-w-[82%] rounded-[var(--radius-sm)] bg-[var(--accent)] p-3 text-sm text-black" : "max-w-[82%] rounded-[var(--radius-sm)] bg-white/8 p-3 text-sm"}>
      {body}
    </p>
  );
}
